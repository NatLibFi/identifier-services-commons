...
=======
  ...
  const {formatDetails={}} = values;

  const requiredFields = ['name', 'publisherEmail', 'title', 'publicationTime', 'type', 'authorGivenName', 'authorFamilyName', 'role'];
  requiredFields.forEach(field => {
    if (!values[field]) {
      errors[field] = 'Required';
    }
  ...
  if(isEmpty(formatDetails.fileFormat)) {
    errors.formatDetails.fileFormat = 'Required';
  }
